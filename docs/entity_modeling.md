...
| created_at | TIMESTAMPTZ | 가입 일시 | NOT NULL, DEFAULT now() ||
| deleted_at | TIMESTAMPTZ | 탈퇴 일시 | NULL ||

## 2. 모임
### meetings
| **설명** | 반복 모임의 기본 정보와 운영 단위 설정을 저장하는 테이블 |
| --- | --- |
| **조회 규칙** | 운영자 홈/모임 목록에서는 `owner_id` 기준으로 조회하고, 기본 목록은 `archived_at IS NULL`인 활성 모임을 최신 수정 순으로 보여 준다. |
| **인덱스** | `PRIMARY KEY (meeting_id)`<br>`INDEX idx_meetings_owner_active (owner_id, created_at, updated_at DESC)` |
| **유니크** | 없음 |

| 컬럼명 | 타입 | 한국말 컬럼명 | 제약 조건 | 설명 |
| --- | --- | --- | --- | --- |
| id | BIGINT | 모임 ID | PK ||
| owner_id | BIGINT | 운영자 ID | NOT NULL, FK(`users.id`) ||
...
| updated_at | TIMESTAMPTZ | 수정 시각 | NOT NULL, DEFAULT now() ||
...
| joined_at | TIMESTAMPTZ | 참여 일시 | NOT NULL, DEFAULT now() ||
| left_at | TIMESTAMPTZ | 이탈 일시 | NULL | 탈퇴/제외 시각 |
...
| created_at | TIMESTAMPTZ | 생성 시각 | NOT NULL, DEFAULT now() ||
| expires_at | TIMESTAMPTZ | 만료 시각 | NOT NULL ||
